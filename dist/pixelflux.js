...
const _t = "26.1-pre-3", F = `https://raw.githubusercontent.com/InventivetalentDev/minecraft-assets/${_t}/assets/minecraft`, K = {
  ...
  render() {
    ...
    (this.canvas.width !== s || this.canvas.height !== t) && this.renderer.setSize(s, t, !1), this.renderer.setScissorTest(!1), this.renderer.clear(), this.renderer.setScissorTest(!0), this.views.forEach((e) => {
...
async function bt(s) {
  ...
  for (const [h, E] of Object.entries(n))
    if (E && !E.startsWith("#")) {
      const w = W(E), O = `${i}/textures/${w}.png`, T = `${F}/textures/${w}.png`;
      l.push(
        H(O).then((f) => f || (console.warn(`기본 텍스처 로드 실패, fallback 시도: ${T}`), H(T))).then((f) => {
          r[h] = f;
        })
      );
    }
  ...
  return o.forEach((h, E) => {
    const w = E * 1e-4, O = (h.to[0] - h.from[0]) / 16, T = (h.to[1] - h.from[1]) / 16, f = (h.to[2] - h.from[2]) / 16, L = new p.BoxGeometry(O, T, f), D = [], B = ["east", "west", "up", "down", "south", "north"], P = L.attributes.uv;
    B.forEach((k, V) => {
      if (h.faces && h.faces[k]) {
        const y = h.faces[k], G = y.texture.replace("#", ""), j = r[G];
        if (j) {
          if (D.push(new p.MeshLambertMaterial({
            map: j,
            transparent: !0,
            alphaTest: 0.1
          })), y.uv) {
          ...
          D.push(new p.MeshBasicMaterial({ color: 16711935 }));
      } else
        D.push(new p.MeshBasicMaterial({ visible: !1 }));
    }), P.needsUpdate = !0;
    const A = new p.Mesh(L, D);
    A.position.set(
